...
parser = argparse.ArgumentParser("g1_examples")
parser.add_argument(
    "--behavior",
    type=str,
    default="block_stacking_behavior",
    help="Which behavior to run. See behavior/g1 for available behavior files.",
)
...
import numpy as np
from behaviors.g1.g1_behaviors import ContextStateMonitor, behaviors
...
from isaacsim.cortex.framework.robotg1 import add_g1_to_stage
...
def main():
    ...
    # Load the g1 robot
    # Adiciona o robô g1 ao mundo
    robot = world.add_robot(add_g1_to_stage(
        name="g1",
        prim_path="/World/g1",
        usd_path="/home/leo/ws_isaacsim/unitree/src/unitree_g1_description/usd/g1_world.usd"
    ))
    ...
    for i, (x, spec) in enumerate(zip(np.linspace(0.2, 0.56, len(obs_specs)), obs_specs)):
        obj = world.scene.add(
            DynamicCuboid(
