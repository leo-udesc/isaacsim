...
class EGH80Gripper(CortexGripper):
    """EGH80 specific parallel gripper.
...
class DEX31Gripper(CortexGripper):
    """DEX31 specific tree fingers hand gripper.

    
    """
    #"right_hand_thumb_0_joint", "right_hand_thumb_1_joint", "right_hand_index_0_joint","right_hand_middle_0_joint",
    def __init__(self, articulation: SingleArticulation):
        super().__init__(
            articulation_subset=ArticulationSubset(
                articulation, ["right_hand_thumb_1_joint",
    ...
    def joints_to_width(self, joint_positions: Sequence[float]) -> float:
        """The width is NOT simply the sum of the two prismatic joints.

        Args:
            joint_positions: The values for joints 
                            ["right_hand_thumb_1_joint", "right_hand_thumb_2_joint", 
                            "right_hand_index_0_joint",  "right_hand_index_1_joint",
                            "right_hand_middle_0_joint", "right_hand_middle_1_joint"].

        Returns:
            The width of the gripper DON'T corresponding to those joint positions.
            WE NEED TO IMPLEMENT THIS FUNCTION BETTER.
        """
        
        return joint_positions[0] + joint_positions[1] + joint_positions[2] + joint_positions[3] + joint_positions[4] + joint_positions[5]

    def width_to_joints(self, width: float) -> np.ndarray:
        """Cada junta NÃO tem metade da largura. 

            Args:
                width: A largura da pinça

            Returns:
                Os valores para as juntas ["right_hand_thumb_1_joint", "right_hand_thumb_2_joint", 
                            "right_hand_index_0_joint",  "right_hand_index_1_joint",
                            "right_hand_middle_0_joint", "right_hand_middle_1_joint"] que fornecem a
                largura da pinça solicitada. SERÁ NECESSÁRIO IMPLEMENTAR MELHOR ESSA FUNÇÃO.
        """

        return np.array([width / 2, width / 2, width / 2,width / 2, width / 2, width / 2])
...
class CortexUR3e(MotionCommandedRobot):
    """O Cortex ur3e contém comandantes para comandar o efetor final 
        ...
        Este objeto envolve apenas um USD UR3e existente no estágio no prim_path especificado.
        Para adicioná-lo ao estágio primeiro e depois envolvê-lo,
        use o método add_ur3e_to_stage().

        Observe que position e orientation são ambos relativos ao prim em que o UR3e está.

        Argumentos:
        name: Um nome para o robô UR3e. Os robôs adicionados ao CortexWorld devem ter nomes exclusivos.
        prim_path: O caminho para o prim no estágio USD.
    ...
    def __init__(
        ...
        use_motion_commander=True,
         ...
         # Carrega a configuração da política de movimento para o UR3e usando o nome do robô e o nome da política.
        motion_policy_config = icl.load_supported_motion_policy_config("UR3e", "RMPflow")
        # Inicializa a classe base MotionCommandedRobot com os parâmetros fornecidos e a configuração da política.
        super().__init__(
            name=name,
            prim_path=prim_path,
            motion_policy_config=motion_policy_config,  
            ...
            orientation=orientation,
            settings=MotionCommandedRobot.Settings(
                active_commander=use_motion_commander,  # Define se o motion commander estará ativo.
                smoothed_rmpflow=True, # Habilita o uso de RMPflow suavizado. (menor jerk)
                smoothed_commands=True  # Habilita o uso de comandos suavizados.
            ),
        )
        # Cria um comandante de garra EGH80 e o adiciona ao robô com o nome "gripper".
        self.gripper_commander = EGH80Gripper(self)
        self.add_commander("gripper", self.gripper_commander)

    def initialize(self, physics_sim_view: omni.physics.tensors.SimulationView = None) -> None:
        """Inicializa usando o initialize() do MotionCommandedRobot e também adiciona 
        ...
        kds = np.array([50000.0, 50000.0, 50000.0, 30000.0, 30000.0, 30000.0, 100.0, 100.0])
        if verbose:
...
def add_ur3e_to_stage(
    ...
    if usd_path is not None:
        ...
        usd_path = get_assets_root_path_or_die() + "/Isaac/Robots/UniversalRobots/ur3e/ur3e.usd"
        add_reference_to_stage(usd_path=usd_path, prim_path=prim_path)

    return CortexUR3e(name, prim_path, position, orientation)

#=========================================================================================
class CortexG1(MotionCommandedRobot):
     """The Cortex G1 contains commanders for commanding the "hand-effector" (a MotionCommander
     governing the full arm) and the gripper (a DEX3_1Grippers governing the fingers).
     ...
     Args:
         name:  O nome para o robô G1. Todos os robôs adicionados ao CortexWorld devem ter nomes exclusivos.
         prim_path: O caminho para o prim G1 no stage USD.
         position: A posição do robô. Consulte o parâmetro position do CortexRobot para obter detalhes.
         orientation:  A orientação do robô. Consulte o parâmetro orientation do CortexRobot para obter detalhes.
         use_motion_commander: Quando definido como True (padrão), usa o comandante de movimento. 
                                Caso contrário, inclui apenas um DirectSubsetCommander para o braço.
     """
     ...
     def initialize(self, physics_sim_view: omni.physics.tensors.SimulationView = None) -> None:
         ...
         verbose = True 
         kps = np.array([1000000.0, 1000000.0, 1000000.0, 1000000.0, 1000000.0, 1000000.0, 1000000.0, 1000000.0, 1000.0, 1000.0, 1000.0,1000.0,1000.0,1000.0])
         kds = np.array([50000.0, 50000.0, 50000.0, 50000.0, 50000.0, 30000.0, 30000.0, 30000.0, 100.0, 100.0, 100.0,100.0,100.0,100.0])
         if verbose:
...
def add_g1_to_stage(
     name: str,
     prim_path: str,
     usd_path: Optional[str] = "/home/leo/ws_isaacsim/g1/usd/g1.usd",
     position: Optional[Sequence[float]] = None,
     orientation: Optional[Sequence[float]] = None,
     use_motion_commander=True,
):
    ...
    if usd_path is not None:
        ...
        usd_path = get_assets_root_path_or_die() + "/Isaac/Robots/G1/g1.usd"
        add_reference_to_stage(usd_path=usd_path, prim_path=prim_path)
