...
class DfApproachTarget(DfDecider):
    ...
    def __init__(
        self,
        approach_along_axis: Optional[int] = 2,  #aproximação ao longo do eixo z
        direction_length: Optional[float] = 0.1, #comprimento da direção de aproximação
    ...
    def decide(self) -> DfDecision:
        ...
        # Define o eixo de aproximação com base no nome do robô.
        # Se o nome do robô contiver "g1",usa o eixo X (0) ou  usa o eixo Y (1).
        # Caso contrário, usa o eixo Z (2) como padrão.
        if hasattr(self, "context") and self.context and hasattr(self.context, "robot") and "g1" in self.context.robot.name.lower():
            self.approach_along_axis = 1
        else:
            self.approach_along_axis = 2
...
class DfCloseGripper(DfAction):
    ...
    Supports sending this decider node the width parameter from a parent. If it comeDfCloseGrippers from a parent
...
class DfMoveGripper(DfAction):
    ...
    Args:
        width: The width to move the gripper to. This value is optional since a parent node can send
            the parameter through its decision params.DfCloseGripper
    """
